...
const app = {
    ...
    openAISettings: function () {
        ...
        list.innerHTML = '';
        const providers = [
            { id: 'openai',    label: 'OpenAI',    model_placeholder: 'gpt-4o' },
            { id: 'anthropic', label: 'Anthropic', model_placeholder: 'claude-3-5-sonnet-20241022' },
            { id: 'minimax',   label: 'Minimax',   model_placeholder: 'MiniMax-Text-01' },
            { id: 'zhipu',      label: 'Zhipu GLM', model_placeholder: 'glm-4-flash' },
            { id: 'qwen',      label: 'Qwen',      model_placeholder: 'qwen-plus' },
            { id: 'deepseek',  label: 'DeepSeek',  model_placeholder: 'deepseek-chat' },
            { id: 'gemini',    label: 'Gemini',     model_placeholder: 'gemini-1.5-flash' },
        ];
        providers.forEach(p => {
            list.innerHTML += `
                <div style="margin-bottom:12px;">
                    <label style="font-size:0.85rem;font-weight:500;">${p.label}</label>
                    <input type="password" id="key_${p.id}" placeholder="API Key" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:6px;margin-top:4px;box-sizing:border-box;" />
                    <input type="text" id="url_${p.id}" placeholder="Base URL (optional)" style="width:100%;padding:8px;border:1px solid #e2e8f0;border-radius:6px;margin-top:4px;box-sizing:border-box;" />
                </div>`;
        });
        // Load current keys from server
        fetch('/api/ai/keys')
            .then(r => r.json())
            .then(data => {
                if (data.status !== 'success') return;
                Object.entries(data.providers).forEach(([k, v]) => {
                    const keyEl = document.getElementById('key_' + k);
                    const urlEl = document.getElementById('url_' + k);
                    if (keyEl) keyEl.value = v.has_key ? '••••••••' : '';
                    if (urlEl) urlEl.value = v.base_url || '';
                });
            });
    },

    closeAISettings: function () {
        document.getElementById('modal-ai-settings').style.display = 'none';
    },

    saveAIKeys: function () {
        const providers = ['openai', 'anthropic', 'minimax', 'zhipu', 'qwen', 'deepseek', 'gemini'];
        const payload = {};
        providers.forEach(p => {
            ...
            if (keyVal && keyVal !== '••••••••') {
                payload[p] = { api_key: keyVal, base_url: urlVal || undefined };
            }
        });
        fetch('/api/ai/keys', {
            method: 'POST',
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    alert('Failed to save keys: ' + (data.message || 'Unknown error'));
                }
            })
            .catch(e => alert('Network error saving keys: ' + e));
    },

    // ── AI File Parse Modal ─────────────────────────────────────
    openAIFileParse: function () {
        if (!this.state.sessionId) {
            alert('No active session. Please create or load a session first.');
            return;
        }
        document.getElementById('modal-ai-parse').style.display = 'flex';
        document.getElementById('parse-preview').style.display = 'none';
        document.getElementById('parse-status').style.display = 'none';
        document.getElementById('parse-file-list').innerHTML = '';
        document.getElementById('btn-run-parse').disabled = true;
    },

    closeAIFileParse: function () {
        document.getElementById('modal-ai-parse').style.display = 'none';
    },

    handleParseFilesSelect: function (files) {
        const list = document.getElementById('parse-file-list');
        list.innerHTML = '';
        Array.from(files).forEach(f => {
            list.innerHTML += `<div style="font-size:0.85rem;padding:4px 0;border-bottom:1px solid #f0f0f0;">📄 ${this.escapeHtml(f.name)} <span style="color:var(--text-muted);">(${(f.size/1024).toFixed(1)} KB)</span></div>`;
        });
        document.getElementById('btn-run-parse').disabled = files.length === 0;
    },

    runAIFileParse: async function () {
        const files = document.getElementById('parse-files').files;
        if (!files.length) return;
        const provider = document.getElementById('parse-provider').value;
        const model = document.getElementById('parse-model').value.trim() || 'gpt-4o';
        const status = document.getElementById('parse-status');
        const preview = document.getElementById('parse-preview');
        const previewContent = document.getElementById('parse-preview-content');

        status.style.display = 'block';
        status.innerHTML = '🧠 AI is parsing files... please wait.';
        status.style.color = 'var(--text-secondary)';

        // Build field schema from current form
        const fieldSchema = {};
        document.querySelectorAll('[id^="input_"]').forEach(el => {
            const fc = el.id.replace('input_', '');
            const note = el.closest('.field-item')?.querySelector('.note')?.innerText || '';
            const exampleValue = el.placeholder || '';
            const label = el.closest('.field-item')?.querySelector('label')?.innerText || fc;
            if (fc) fieldSchema[fc] = { label, note, exampleValue };
        });
        ...
        formData.append('field_schema', JSON.stringify(fieldSchema));

        try {
            const res = await fetch('/api/ai/parse_files', { method: 'POST', body: formData });
            const json = await res.json();
            status.style.display = 'none';

            if (json.status !== 'success') {
                alert('Parse failed: ' + (json.message || 'Unknown error'));
                return;
            }

            const parsed = json.parsed || {};
            const errorKeys = Object.keys(parsed).filter(k => k.startsWith('_error_'));

            // Show preview
            let previewHtml = '<table style="width:100%;font-size:0.85rem;border-collapse:collapse;">';
            previewHtml += '<tr style="background:#f7fafc;"><th style="padding:6px 8px;text-align:left;">Field</th><th style="padding:6px 8px;text-align:left;">Extracted Value</th></tr>';
            Object.entries(parsed).forEach(([k, v]) => {
                if (!k.startsWith('_error_')) {
                    previewHtml += `<tr><td style="padding:6px 8px;border-bottom:1px solid #f0f0f0;">${this.escapeHtml(k)}</td><td style="padding:6px 8px;border-bottom:1px solid #f0f0f0;">${this.escapeHtml(String(v))}</td></tr>`;
                }
            });
            previewHtml += '</table>';
            if (errorKeys.length) {
                previewHtml += `<p style="color:var(--accent);font-size:0.85rem;margin-top:10px;">⚠️ ${errorKeys.length} file(s) failed to parse. Check console for details.</p>`;
            }
            ...
            let applied = 0;
            Object.entries(parsed).forEach(([k, v]) => {
                if (!k.startsWith('_error_') && k in fieldSchema) {
            ...
            if (applied > 0) {
                status.style.display = 'block';
                status.innerHTML = `✅ Applied ${applied} field(s) from AI parsing. Review and Save.`;
                status.style.color = 'green';
                this.saveDraft();
            }
        } catch (e) {
            status.style.display = 'block';
            status.innerHTML = '❌ Error: ' + this.escapeHtml(String(e));
            status.style.color = 'var(--accent)';
        }
    },

    // ── AI Compliance Check Modal ───────────────────────────────
    runAIComplianceCheck: function () {
        if (!this.state.sessionId) {
            alert('No active session. Please create or load a session first.');
            return;
        }
        document.getElementById('modal-ai-check').style.display = 'flex';
    ...
    closeAIComplianceCheck: function () {
        document.getElementById('modal-ai-check').style.display = 'none';
    },

    runAIComplianceCheckGo: async function () {
        const provider = document.getElementById('check-provider').value;
        const model = document.getElementById('check-model').value.trim() || 'gpt-4o';
        ...
        try {
            const res = await fetch('/api/ai/compliance_check', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            if (json.status !== 'success') {
                results.innerHTML = `<p style="color:var(--accent);">Check failed: ${this.escapeHtml(json.message || 'Unknown error')}</p>`;
            ...
            let html = '';
            if (!issues.length) {
                html = '<p style="color:green;">✅ No major compliance issues found. All critical fields appear to be in order.</p>';
            } else {
                const p1 = issues.filter(i => i.priority === 'P1');
                const p2 = issues.filter(i => i.priority === 'P2');
                const p3 = issues.filter(i => i.priority === 'P3');

                html = `<p style="margin-bottom:15px;">共发现 <strong>${issues.length}</strong> 个问题：<span style="color:var(--accent);">P1=${p1.length}</span> / <span style="color:#d69e2e;">P2=${p2.length}</span> / <span style="color:var(--text-muted);">P3=${p3.length}</span></p>`;

                const makeSection = (label, color, items) => {
                    if (!items.length) return '';
                    let s = `<div style="margin-bottom:15px;">
                        <div style="font-weight:600;color:${color};margin-bottom:6px;">${label} (${items.length})</div>
                        <div style="display:grid;gap:8px;">`;
                    items.forEach(i => {
                        s += `<div style="border:1px solid #e2e8f0;border-radius:6px;padding:10px;background:#f7fafc;">
                            <div style="font-weight:500;">${this.escapeHtml(i.label || i.field)} <span style="font-size:0.75rem;background:${color};color:white;padding:1px 6px;border-radius:10px;margin-left:5px;">${i.priority}</span></div>
                            <div style="font-size:0.85rem;color:var(--text-secondary);margin-top:4px;">${this.escapeHtml(i.issue)}</div>
                            <div style="font-size:0.85rem;color:var(--accent);margin-top:4px;">💡 ${this.escapeHtml(i.suggestion || '')}</div>
                        </div>`;
                    });
                    s += '</div></div>';
                    return s;
                ...
                html += makeSection('🔴 P1 — 阻断性问题（申报被拒风险）', '#e53e3e', p1);
                html += makeSection('🟡 P2 — 重要字段（需尽快补充）', '#d69e2e', p2);
                html += makeSection('⚪ P3 — 优化建议', 'var(--text-muted)', p3);
            }
            ...
            loading.style.display = 'none';
            results.innerHTML = `<p style="color:var(--accent);">Error: ${this.escapeHtml(String(e))}</p>`;
            results.style.display = 'block';
        } finally {
            document.getElementById('btn-run-check').disabled = false;
        }
